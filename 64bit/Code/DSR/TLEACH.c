...
int fn_NetSim_TLEACH_GetNextHop(NetSim_EVENTDETAILS* pstruEventDetails)
{
	...
	for (i=0; i<NUMBEROFCLUSTERS ; i++)
	{
		if(CHcount[i] == 4)
		{
			CH [i]= fn_NetSim_TLEACH_AssignClusterHead(i,pstruDevicePower);
			prevCH[i] = CH[i];
			CHcount[i] = 0;
		}
		else
...
int fn_NetSim_TLEACH_AssignClusterHead (int ClusterNumber,POWER **pstruDevicePower)
//Function to dynamically assign Cluster Heads based on the sensors' remaining energy.
{
	...
	for(i=0; i<SIZEOFCLUSTERS; i++)
	{
		...
		DevPower = pstruDevicePower[DeviceId - 1]->dRemainingPower;
	...
	if (ClusterHeadID == 0)
	{
		fn_NetSim_TLEACH_set_threshold(pstruDevicePower, ClusterNumber);
		fn_NetSim_TLEACH_AssignClusterHead(ClusterNumber, pstruDevicePower);
	}
	...
	for (i = 0; i < NETWORK->nDeviceCount; i++)
	{
		...
		double energy = pstruDevicePower[i]->dRemainingPower;
		if (pstruEventDetails->dEventTime == 0)
			energy = pstruDevicePower[i]->dInitialEnergy_mW;
...
int fn_NetSim_TLEACH_init()
{
	...
	for (i = 0; i < NUMBEROFCLUSTERS; i++)
	{
		fn_NetSim_TLEACH_set_threshold(pstruDevicePower, i);
	}
	return 1;
}

//Function to calculate the energy threshold value for each cluster based on the THRESHOLD_PROPORTION
int fn_NetSim_TLEACH_set_threshold(POWER** pstruDevicePower, int cl_id)
{
	int i = 0;
	CHthreshold[cl_id] = 0;
	for (i = 0; i < SIZEOFCLUSTERS; i++)
	{
		...
		double CurrDevPower = pstruDevicePower[DeviceId - 1]->dRemainingPower;
		if(pstruEventDetails->dEventTime==0)
			CurrDevPower = pstruDevicePower[DeviceId - 1]->dInitialEnergy_mW;
