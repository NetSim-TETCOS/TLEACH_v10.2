...
_declspec(dllexport) int fn_NetSim_DSR_Metrics(PMETRICSWRITER filename)
{
	...
	//This #ifdef.....#endif block defines the TLEACH Metrics, which shows the packets 
	...
	//To enable TLEACH, uncomment the line "#define _TLEACH" in DSR.h
    #ifdef _TLEACH		
		PMETRICSNODE table = init_metrics_node(MetricsNode_Table, "TLEACH_Metrics", NULL);
		...
		PMETRICSNODE menu = init_metrics_node(MetricsNode_Menu, "TLEACH_METRICS", NULL);
